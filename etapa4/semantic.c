...
void checkDataTypes(AST_NODE *node)
{
	...
	switch (node->type) 
	{
		...
		case AST_PARENTESES:
			...
			break;
		case AST_ATRIBUICAO_VAR:
			if ((!(verifyAttributionTypes(node->symbol->dataType, node->sons[0]->dataType))) && (!((node->symbol->dataType)==13 && (node->sons[0]->dataType==17))))
			{
				semanticError(node->lineNumber, "Tipos conflitantes para atribuição.");
				printf("%d\n", node->symbol->dataType);
				printf("%d\n", node->sons[0]->dataType);
				printf("%d\n", node->symbol->type);
				printf("%d", node->sons[0]->type);
	 		}
			break;
		case AST_ATRIBUICAO_VETOR:
		...
		case AST_IF:
			...
			{
				semanticError(node->sons[0]->lineNumber, "Comando IF - esperado booleano.");
			}
			break;
		case AST_WHILE:
			if (node->sons[0]->dataType != DATATYPE_BOOLEANO)
